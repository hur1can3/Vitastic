...
namespace VitasticCore.AspNet.Auth;
...
public static class AuthServiceCollectionExtensions
{
    ...
    public static void AddAuthorizationPoliciesFromSettings(this IServiceCollection services, AuthorizationSettings authorizationSettings)
    {
        ...
        {
            foreach ((var key, var value) in authorizationSettings.Policies)
            {
    ...
    {
        _ = services.AddControllers(options => options.Filters.Add(new AuthorizeFilter(policy)));
    }
    ...
    {
        _ = services.AddSingleton<ICurrentUserAccessor, WebCurrentUserAccessor>();
    }
    ...
    {
        _ = services.AddAuthentication(HttpSysDefaults.AuthenticationScheme);

        if (environment.IsDevelopment())
        {
            _ = services.AddControllers(options => options.Filters.Add(new AllowAnonymousFilter()));
        }
    }
}
