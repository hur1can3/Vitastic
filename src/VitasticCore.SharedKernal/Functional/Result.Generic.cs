...
public sealed class Result<T> : ResultAbstract, IResult<T>
{
    ...
    {
        _ = value.EnsureNotNull("Cannot set a result value of null. Use non-generic Result for results without values.");
    ...
    [NotNull]
    public T Value => IsFailed ? throw new InvalidOperationException("Do not access the value of Result if it is failed.") : _value!.Value;
}
