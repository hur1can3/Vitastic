...
public sealed class Maybe<T> : IEquatable<Maybe<T>>
{
    ...
    [NotNull]
    public T Value => HasNoValue ? throw new InvalidOperationException("Do not access the value of Maybe if it has no value.") : _value!.Value;
    ...
    public static bool operator ==(Maybe<T> maybe, T value)
    {
        return ReferenceEquals(maybe, value) || (maybe is not null && !maybe.HasNoValue && maybe.Value!.Equals(value));
    }
...
&& ((HasNoValue && other.HasNoValue) || (!HasNoValue && !other.HasNoValue && Value.Equals(other._value!.Value)));
    }
